...
// ParaAG.cpp : Defines the entry point for the console application.
#include <iostream>
#include <string>
#include <sstream>
#include <fstream>
...
int main(int argc, char* argv[])
{
	...
	for (int i = 1; i < argc; i += 2) {
		...
		else if (optstr == "-convexity") {
			params.convex_mu = std::stof(valstr);
		}
		else if (optstr == "-display") {
			params.display = (std::stoi(valstr) > 0);
		}
		else {
			std::cout << "ParaAG: Invalid arguments, please try again." << std::endl;
			std::exit(0);
		}
	}
	if (must_count < 4) {
		std::cout << "ParaAG arguments: -data <string> -loss <char> -lambda <double> -regu <char>" << std::endl;
		...
		std::cout << "                  -convexity <double> -rownorm <double> -display <int>" << std::endl;
	...
	params.filename = data_name + "_" + std::string(1, loss_type) + std::string(1, regu_type) + "_acclgd" 
	...
	// ADMM algorithm ------------------------------------------------------------------------------
	MPI_Barrier(comm_world);
	acclgd_mpi(comm_world, N, X, y, *f, lambda, *g, w, params);
