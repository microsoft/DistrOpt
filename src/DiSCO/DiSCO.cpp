...
//int main_disco_mpi(int argc, char* argv[])
int main(int argc, char* argv[])
{
	...
	for (int i = 1; i < argc; i += 2) {
		...
		if (optstr == "-data") {
			...
			params.linesearch = (std::stoi(valstr) > 0);
		}
		else if (optstr == "-epsobj") {
			params.eps_obj = std::stof(valstr);
		}
		else if (optstr == "-spcsolver") {
			params.spc_solver = valstr[0];
		}
		else if (optstr == "-spceps") {
			params.spc_eps = std::stof(valstr);
		}
		else if (optstr == "-spcpass") {
			params.spc_pass = std::stoi(valstr);
		}
		else if (optstr == "-beta") {
			params.beta = std::stof(valstr);
		}
		...
		else if (optstr == "-pcgitrs") {
			params.pcg_itrs = std::stoi(valstr);
		}
		else if (optstr == "-pcgmu") {
			params.pcg_mu = std::stof(valstr);
		}
		else if (optstr == "-pcgadpt") {
			params.pcg_adpt = (std::stoi(valstr) > 0);
		}
		else if (optstr == "-pcgrec") {
			params.pcg_record = (std::stoi(valstr) > 0);
		}
		else if (optstr == "-pcgpolak") {
			params.pcg_polak = (std::stoi(valstr) > 0);
		}
			...
			std::cout << "DiSCO: Invalid arguments, please try again." << std::endl;
			std::exit(0);
		}
	}
	if (must_count < 4) {
	...
	ssmu << std::scientific << std::setprecision(1) << params.pcg_mu;
	std::string smu = ssmu.str();
	smu.erase(smu.find_first_of("."), 2);
	std::string time_comm = params.pcg_record ? "_comm" : "_time";
	params.filename = data_name + "_" + std::string(1, loss_type) + std::string(1, regu_type) + "_disco_"
		+ std::string(1, params.spc_solver) + std::to_string(params.spc_pass) + "_" + slambda + "_mu" + smu + "_" + init_mthd 
		+ time_comm;
	...
	// CoCoA algorithm ------------------------------------------------------------------------------
	MPI_Barrier(comm_world);
	disco_mpi(comm_world, N, X, y, *f, lambda, g, w, params);
